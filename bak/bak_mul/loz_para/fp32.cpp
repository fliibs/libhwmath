...
Fp32 Fp32::mul(const Fp32 &rhs){
    ...
    if((temp_mant&mantissa_48_bit)&&(!(temp_expo&exponent_sign))){                                                             //1x.xxxxxxx
        ...
        if(!(temp_expo&exponent_sign) && ((temp_expo&exponent_num)>zero_nums) && temp_mant){         //0.xxxxxx and normailized
...
uint32_t Fp32::get_mask(uint32_t mantissa_in){
    ...
    for(int i= 0 ; i < 24; i++) {
        ...
        for (int j = 0; j < i; j++)
        {   
            // debug_printf("binary before %d is %b,top_binary is %b",i,top_binary & mantissa_in,top_binary);
            or_i          =    or_i || (top_binary & mantissa_in) ;
            top_binary    =    top_binary>>1                    ;
        }
        mani        =   ((!or_i) && (mantissa_in & cur_binary))? 1: 0  ;
        mask        =    (mask<<1) + mani                                ;
        cur_binary >>=  1                                              ;
        // debug_printf("i is %d,or_i is %u,mani is %b",i,or_i,mani);
    }
    return mask;
}
...
{    
    ...
    debug_printf("mant_msk is %lb",mantissa_mask);
